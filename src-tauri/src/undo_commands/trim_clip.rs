// src-tauri/src/undo_commands/trim_clip.rs
//! TrimClipCommand - Reversible clip trimming

use crate::reversible_command::{memory_size_of_string, ReversibleCommand};
use crate::timeline::TimelineState;

/// Command to trim a clip's start and/or end
#[derive(Debug, Clone)]
pub struct TrimClipCommand {
    /// ID of the clip to trim
    clip_id: String,
    /// Delta to apply to start time (positive = trim from start)
    trim_start_delta: Option<f64>,
    /// Delta to apply to duration (negative = trim from end)
    trim_end_delta: Option<f64>,

    // Captured state for undo
    /// Original start time
    old_start: Option<f64>,
    /// Original duration
    old_duration: Option<f64>,
}

impl TrimClipCommand {
    /// Create a new trim command
    pub fn new(
        clip_id: String,
        trim_start_delta: Option<f64>,
        trim_end_delta: Option<f64>,
    ) -> Self {
        Self {
            clip_id,
            trim_start_delta,
            trim_end_delta,
            old_start: None,
            old_duration: None,
        }
    }
}

impl ReversibleCommand for TrimClipCommand {
    fn execute(&mut self, state: &mut TimelineState) -> Result<(), String> {
        // Find the clip
        let clip = state
            .get_clip_by_id_mut(&self.clip_id)
            .ok_or_else(|| format!("Clip '{}' not found", self.clip_id))?;

        // Capture old values for undo
        self.old_start = Some(clip.start);
        self.old_duration = Some(clip.duration);

        // Apply trim_start_delta
        if let Some(delta) = self.trim_start_delta {
            clip.start += delta;
            clip.duration -= delta;
        }

        // Apply trim_end_delta
        if let Some(delta) = self.trim_end_delta {
            clip.duration += delta;
        }

        // Recalculate duration
        state.recalculate_duration();

        // Validate invariants
        state.validate_invariants()?;

        Ok(())
    }

    fn undo(&mut self, state: &mut TimelineState) -> Result<(), String> {
        let old_start = self.old_start.ok_or("Cannot undo: no old start captured")?;
        let old_duration = self
            .old_duration
            .ok_or("Cannot undo: no old duration captured")?;

        // Find the clip
        let clip = state
            .get_clip_by_id_mut(&self.clip_id)
            .ok_or_else(|| format!("Clip '{}' not found", self.clip_id))?;

        // Restore old values
        clip.start = old_start;
        clip.duration = old_duration;

        // Recalculate duration
        state.recalculate_duration();

        // Validate invariants
        state.validate_invariants()?;

        Ok(())
    }

    fn memory_size(&self) -> usize {
        std::mem::size_of::<Self>() + memory_size_of_string(&self.clip_id)
    }

    fn description(&self) -> String {
        format!(
            "Trim clip '{}' (start: {:?}, end: {:?})",
            self.clip_id, self.trim_start_delta, self.trim_end_delta
        )
    }

    fn type_name(&self) -> &'static str {
        "TrimClipCommand"
    }

    fn target_clip_id(&self) -> Option<&str> {
        Some(&self.clip_id)
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::timeline::Clip;

    fn make_test_clip(id: &str) -> Clip {
        Clip {
            id: id.to_string(),
            track_id: "track-1".to_string(),
            start: 0.0,
            duration: 10.0,
            source_file: "/test.mp4".to_string(),
        }
    }

    #[test]
    fn test_trim_start_execute_and_undo() {
        let mut state = TimelineState::new();
        state.add_clip(make_test_clip("clip-1"));

        // Execute trim start
        let mut cmd = TrimClipCommand::new("clip-1".to_string(), Some(2.0), None);
        assert!(cmd.execute(&mut state).is_ok());

        let clip = state.get_clip_by_id("clip-1").unwrap();
        assert_eq!(clip.start, 2.0);
        assert_eq!(clip.duration, 8.0);

        // Undo trim
        assert!(cmd.undo(&mut state).is_ok());

        let clip = state.get_clip_by_id("clip-1").unwrap();
        assert_eq!(clip.start, 0.0);
        assert_eq!(clip.duration, 10.0);
    }

    #[test]
    fn test_trim_end_execute_and_undo() {
        let mut state = TimelineState::new();
        state.add_clip(make_test_clip("clip-1"));

        // Execute trim end
        let mut cmd = TrimClipCommand::new("clip-1".to_string(), None, Some(-2.0));
        assert!(cmd.execute(&mut state).is_ok());

        let clip = state.get_clip_by_id("clip-1").unwrap();
        assert_eq!(clip.start, 0.0);
        assert_eq!(clip.duration, 8.0);

        // Undo trim
        assert!(cmd.undo(&mut state).is_ok());

        let clip = state.get_clip_by_id("clip-1").unwrap();
        assert_eq!(clip.start, 0.0);
        assert_eq!(clip.duration, 10.0);
    }
}
